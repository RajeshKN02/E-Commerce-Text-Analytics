...
library(data.table)
library(dplyr)
women_clothes <- fread("https://raw.githubusercontent.com/langl1996/E-commerce-Clothing-NLP/master/women_clothes_cleaned-1.csv",stringsAsFactors = FALSE,na.strings = '')
glimpse(women_clothes)
head(women_clothes)
summary(women_clothes)
sum(is.na(women_clothes))

sapply(women_clothes, function(x)sum(is.na(x)))
women_clothes_cleaned = na.omit(women_clothes)
sapply(women_clothes_cleaned, function(x)sum(is.na(x)))
...
library(gridExtra)
library(grid)
most_reviewed_products <- women_clothes_cleaned %>%
  select(Class.Name) %>%
  filter(Class.Name != '') %>%
  group_by(Class.Name) %>%
  summarise(count = n()) %>%
...
library(ggplot2)
which(women_clothes_cleaned$Department.Name)

phisto <- women_clothes_cleaned %>% filter(!is.na(Department.Name), Department.Name != 'Trend') %>% mutate(Department.Name = factor(Department.Name)) %>% group_by(Department.Name) %>% count(Rating) %>% mutate(perc = n/sum(n))

phisto %>% ggplot(aes(x=Rating, y = perc*100, fill = Department.Name)) + geom_bar(stat = 'identity', show.legend = T) + facet_wrap(~Department.Name) + ylab('Percentage of reviews (%)') + geom_text(aes(label=round(perc*100,2)))


#reviews by age
qplot(Age, data=women_clothes_cleaned, geom="density", alpha=I(0.5), main="Reviewers' Age", xlab="Age", fill=I("lightblue"),  alpha = .3, show.legend = NA)
women_clothes_cleaned$bins <- cut(women_clothes_cleaned$Age, breaks = c(0,20,40,60,80,100), 
labels = c("Centennials(0-20)","Gen Y(21-40)","Gen X(41-60)","Baby Boomers(61-80)","Traditionalists(81-100)"))
age_groups <- women_clothes_cleaned %>%
  select(bins,Age) %>%
  group_by(bins) %>%
  summarise(count = n())
...
clothes=filter(women_clothes_cleaned, Class.Name!='Casual bottoms')
clothes=filter(women_clothes_cleaned, Class.Name!='Chemises')
...
ages %>% filter(Age < 80 & Age> 20) %>% group_by(Age_group) %>% count(Class.Name) %>% ggplot(aes(Class.Name, n, fill = Age_group)) + geom_bar(stat='identity', show.legend = FALSE) + facet_wrap(~Age_group, scales = 'free') + xlab('Department') + ylab('Number of Reviews') + geom_text(aes(label = n), hjust = .45) + coord_flip()
...
cor(nchar(women_clothes_cleaned$Review.Text), women_clothes_cleaned$Rating) # -0.05
cor.test(nchar(women_clothes_cleaned$Review.Text), women_clothes_cleaned$Rating) #alternative hypothesis: true correlation is not equal to 0

##Are longer review associated with more positive feedback
cor(nchar(women_clothes_cleaned$Review.Text), women_clothes_cleaned$Positive.Feedback.Count) #0.1923
##Are better ratings associated with more positive feedback
cor(women_clothes_cleaned$Rating, women_clothes_cleaned$Positive.Feedback.Count) #-0.05
#Most Common Words
